...
class UserController extends MasterDataController {
  ...
  create = async (req, res) => {
    try {
      ...
      const result = await db.query(query, [username, password_hash, full_name, role_id, status || 'active']);
  ...
  update = async (req, res) => {
    try {
      ...
      const oldResult = await db.query('SELECT id, username, full_name, role_id, status FROM users WHERE id = $1', [id]);
      ...
      if (password) {
        ...
        query = `
          UPDATE users 
          SET username = $1, password_hash = $2, full_name = $3, role_id = $4, status = $5, updated_at = CURRENT_TIMESTAMP
          WHERE id = $6 RETURNING id, username, full_name, role_id, status
        `;
        params = [username, password_hash, full_name, role_id, status, id];
      } else {
        query = `
          UPDATE users 
          SET username = $1, full_name = $2, role_id = $3, status = $4, updated_at = CURRENT_TIMESTAMP
          WHERE id = $5 RETURNING id, username, full_name, role_id, status
        `;
  ...
  getAll = async (req, res) => {
    try {
      let { page = 1, limit = 10, search = '', sortBy = 'u.id', order = 'ASC' } = req.query;
      ...
      const dataQuery = `
        SELECT u.id, u.username, u.full_name, u.role_id, r.name as role_name, u.status, u.last_login, u.created_at
        FROM users u
