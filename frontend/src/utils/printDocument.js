  ...
  printWindow.document.write(`
    ...
    <body>
      <div class="letterhead">
        <div>
          <div class="brand">Shune Sein</div>
      ...
      <div class="footer">
        <span>Generated by Shune Sein</span>
...
// Generic list/worksheet print (e.g. a Stock Count sheet) - same letterhead
// and @page setup as openPrintDocument, but a plain configurable table
// instead of a fixed quantity/unit-price/subtotal shape, since a worksheet
// isn't a financial document. Columns marked `blank: true` render an empty
// cell (e.g. "Physical Qty" left for the counter to fill in by hand) even
// though the row itself may carry a value for that key.
export const openPrintList = ({
  documentTypeLabel,
  documentNumber,
  extraMeta = [],
  columns = [],
  ...
  const metaRows = extraMeta.filter((row) => row.value !== undefined && row.value !== null && row.value !== '');

  const headerCells = columns.map((col) => `<th class="${col.align === 'right' ? 'num' : ''}">${escapeHtml(col.label)}</th>`).join('');
  const bodyRows = rows.map((row, index) => `
    <tr>
      <td class="col-index">${index + 1}</td>
      ${columns.map((col) => `<td class="${col.align === 'right' ? 'num' : ''} ${col.blank ? 'blank-cell' : ''}">${col.blank ? '' : escapeHtml(row[col.key] ?? '-')}</td>`).join('')}
    </tr>
  `).join('');
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        table.items td.col-index { color: #9ca3af; width: 28px; }
        table.items td { padding: 8px 10px; border-bottom: 1px solid #e5e7eb; }
        table.items td.blank-cell { border-bottom: 1px solid #9ca3af; }
        table.items tbody tr:nth-child(even) { background: #f8fafc; }
        .remark-block { margin-bottom: 20px; padding: 10px 14px; background: #f8fafc; border-left: 3px solid #2563eb; font-size: 12px; }
        .remark-block .meta-label { margin-bottom: 4px; }
        .footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #e5e7eb; font-size: 10px; color: #9ca3af; display: flex; justify-content: space-between; }
